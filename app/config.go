...
// NewAppConstructor returns a new CoolCatApp given encoding type configs.
func NewAppConstructor() network.AppConstructor {
	return func(val network.ValidatorI) servertypes.Application {
		return NewCoolCatApp(
			val.GetCtx().Logger, dbm.NewMemDB(), nil, true, make(map[int64]bool), val.GetCtx().Config.RootDir,
